...
SPREADSHEET_ID = "1vwcc32QdnWEhsN8iiVAl2fFZpo2T_stu2Bf2mwxFhMU"
...
class GoogleSheetsManager:
    ...
    def connect(self):
        ...
        try:
            ...
            self._create_admins_sheet_if_not_exists()
...
@dp.message(F.text == "🔓 Отвязать Telegram от карты")
async def unlink_account(message: types.Message):
    """Отвязка Telegram ID от карты"""
    success = sheets_manager.unlink_telegram_id(message.from_user.id)

    if success:
        await message.answer(
            "✅ Ваш Telegram успешно отвязан от карты выпускника.\n\n"
            "При следующем запросе статуса вам нужно будет заново ввести ФИО и дату рождения.",
            reply_markup=get_main_menu(sheets_manager.is_admin(message.from_user.username))
        )
    else:
        await message.answer(
            "❌ У вас нет привязанной карты (Telegram ID не найден в базе).",
            reply_markup=get_main_menu(sheets_manager.is_admin(message.from_user.username))
        )
...
def get_main_menu(is_admin: bool = False):
    keyboard = [
        ...
        [KeyboardButton(text="🔓 Отвязать Telegram от карты")],
